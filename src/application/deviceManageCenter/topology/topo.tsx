...
import SlideBox from 'components/SlideLayout';
...
import { deleteDeviceTopo, getDeviceTopo } from '../services/topo';
import { EditContext } from '../editDevice/context';
import './index.less';
  ...
  const columns: ColumnProps<DeviceInfo>[] = [
    {
      ...
      render: (text: string, record: DeviceInfo) => (
        <Button onClick={() => {
          history.push(`/deviceManage/editDevice/${record.deviceId}`)
        }}>{text}</Button>
      )
    },
    {
