...
// 格式方法
// 公共方法
export const transitionJsonToString = (jsonObj: any, callback?: Function) => {
  const originObj = jsonObj;
  // 转换后的jsonObj受体对象
  let _jsonObj: any = '';
  ...
  return _jsonObj || originObj;
};
// callback为数据格式化错误的时候处理函数
export const formatJson = (jsonObj: any, callback?: Function) => {
  if (!jsonObj && callback) {
    callback('请先输入JSON串');
    return null;
  }
  ...
  // 开始处理双引号中的内容，将多余的"去除
  _index.reverse().forEach((item: any, index: number) => {
